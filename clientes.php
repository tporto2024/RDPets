...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'delete') {
    $id = (int)($_POST['id'] ?? 0);
    if ($id) {
        $nome = $pdo->prepare('SELECT nome FROM clientes WHERE id = ?');
        $nome->execute([$id]);
        $nomeCliente = $nome->fetchColumn() ?: "ID $id";
        $pdo->prepare('DELETE FROM clientes WHERE id = ?')->execute([$id]);
        logActivity('Clientes', 'Excluiu cliente', $nomeCliente);
    }
    header('Location: clientes.php?deleted=1');
    exit;
}

logActivity('Clientes', 'Visualizou lista de clientes', $_GET['q'] ?? '');
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-200">
                    ...
                    <th class="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase">Tipo de Negócio</th>
            ...
            <?php foreach ($clientes as $c): ?>
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-5 py-3 text-gray-600"><?= e($c['tipo_negocio'] ?? '-') ?></td>
            ...
            <?php if (empty($clientes)): ?>
                <tr><td colspan="8" class="px-5 py-10 text-center text-gray-400">Nenhum cliente encontrado</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
