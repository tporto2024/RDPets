...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'salvar_neg') {
    ...
    if (!$cliId || !in_array($etapa, $etapas)) {
        ...
        if ($negId) {
            ...
            $pdo->prepare('UPDATE negociacoes SET cliente_id=?,etapa=?,qualificacao=?,valor=?,previsao_fechamento=?,responsavel_id=?,indicacao=? WHERE id=?')
                ->execute([$cliId,$etapa,$qual,$valor,$prev,$resp,$ind,$negId]);
            ...
            $pdo->prepare('INSERT INTO negociacoes (cliente_id,etapa,qualificacao,valor,previsao_fechamento,responsavel_id,indicacao) VALUES (?,?,?,?,?,?,?)')
                ->execute([$cliId,$etapa,$qual,$valor,$prev,$resp,$ind]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'salvar_tarefa') {
    ...
    if ($assunto && $quando && $negId) {
        ...
        $pdo->prepare('INSERT INTO tarefas_log (tarefa_id,acao,para_status,changed_by,changed_ip,payload) VALUES (?,?,?,?,?,?)')
            ->execute([$pdo->lastInsertId(),'criada','aberta',$u['nome'],getClientIP(),
...
$neg = null;
if ($negId) {
    $stmt = $pdo->prepare('SELECT n.*,c.nome AS cliente_nome,c.empresa FROM negociacoes n JOIN clientes c ON c.id=n.cliente_id WHERE n.id=?');
    $stmt->execute([$negId]);
    $neg = $stmt->fetch();
                ...
                <form method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="cliente_id" required
                            ...
                            <option value="<?= $c['id'] ?>"
                                <?= (($neg['cliente_id'] ?? $clienteIdPre) == $c['id']) ? 'selected' : '' ?>>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-0.5">
                            <?= e($log['changed_by']) ?> · <?= date('d/m/Y H:i', strtotime($log['changed_at'])) ?>
                        </p>
                    </div>
                </div>
                <?php endforeach; ?>
